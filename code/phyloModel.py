        ...
        for node in tree.traverse("postorder"):
            if node.is_leaf():
                ...
                for child in node.children:
                    # transition_matrix = torch.mm(self.U * torch.exp(self.D * torch.exp(log_branch[child.name])), self.U_inv).clamp(0.0)
                    node.state *= transition_matrix[child.name].mm(child.state)
                scaler = torch.sum(node.state, 0)
                node.state /= scaler
                # logll += torch.sum(scaler.log() * self.site_counts)
                scaler_list.append(scaler)

        # logll += torch.sum(torch.mm(self.pden.view(-1,4), tree.state).log() * self.site_counts)
